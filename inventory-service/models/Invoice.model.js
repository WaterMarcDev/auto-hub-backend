...
InvoiceSchema.statics.createFrom = async function ({ checkInId, transactionId, snapshot = {}, createdBy, elementSellId, invoiceType = "checkin" }) {
  const inv = new this({ checkIn: checkInId, transaction: transactionId, invoiceData: snapshot, paymentMethod: snapshot.paymentMethod, amount: Number(snapshot.amount ?? 0), taxRate: snapshot.taxRate ?? 0, invoiceDate: snapshot.invoiceDate ? new Date(snapshot.invoiceDate) : undefined, createdBy, elementSell: elementSellId, invoiceType });
  await inv.save();
  return inv;
};
