...
exports.handleCallback = async (req, res) => {
  try {
    ...
    try {
      await platformManager.getAdapter(platform).fetchBusinessAccount(integration);
    } catch (bizErr) {
    ...
    // Register webhook
    try {
      await platformManager.getAdapter(platform).registerWebhook(integration);
    } catch (webhookErr) {
...
exports.platformStatus = async (req, res) => {
  try {
    ...
    // Run a health check
    const health = await platformManager.getAdapter(platform).healthCheck(integration);
